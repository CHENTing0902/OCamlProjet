...
let rec loop f l = match l with
  	|[] -> []
  	|(x,y)::rest -> (f x, y) :: loop f rest 

let change vertex_info vmap emap = 
{	label = vmap vertex_info.label;
 	id = vertex_info.id; 
	outedges = loop emap vertex_info.outedges;
	inedges = loop emap vertex_info.inedges
}
   

let map graph vmap emap = 
 	let result = Hashtbl.create (Hashtbl.length graph.vertices) in
    		Hashtbl.iter (fun key v -> Hashtbl.add result key (change v vmap emap)) graph.vertices;
      let newre = {vertices = result} in
	newre
